...
**用自然语言描述你的需求，AI 自动生成软件架构图，一键构建为可运行代码。**

不需要手动画图——告诉 AI 你要做什么，它帮你设计架构、画出来、写代码。你只需要确认和调整。

**目标用户**：有产品想法但不想写代码的人——PM、创始人、独立开发者——用对话把想法变成可运行的项目。
...
```
描述需求 ──→ AI 讨论方案 ──→ AI 生成架构图 ──→ Build All ──→ AI 写代码
告诉 AI 你要做什么    AI 提问澄清需求    自动画在画布上    波次并行调度    本地 AI CLI 执行
```

1. **描述需求** — 新建对话，用自然语言描述你想做的系统（如"帮我设计一个在线书店"）。AI 会像资深架构师一样提问、澄清需求、讨论方案。
2. **AI 生成架构** — 方案确认后，AI 自动在画布上生成架构图（容器、模块、连线），无需手动拖拽。你也可以导入现有代码库，AI 秒速反向生成架构。
3. **迭代优化** — 继续对话修改架构（"加上支付功能"、"把数据库拆成两个"），AI 增量更新画布。每一轮都可以撤销或重新应用。
4. **一键构建** — 点击 "Build All"，按依赖顺序分波次并行生成代码，实时看到每个模块的构建进度。
