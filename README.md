...
告诉 AI "帮我做一个外卖 App"。它会像资深架构师一样追问你：用户端还是商家端？需要实时配送追踪吗？讨论完毕，架构图自动出现在画布上——容器、模块、连线，全部就位。然后一键构建，AI 按依赖顺序并行生成每个模块的代码。
...
1. 💬 **"帮我做一个外卖 App"** — 一句话开始。AI 像资深架构师一样追问：目标用户是谁？需要哪些核心功能？技术栈有偏好吗？
...
```
"帮我做一个外卖App" ──→ AI 头脑风暴 ──→ 架构图自动生成 ──→ 对话迭代 ──→ 一键构建
