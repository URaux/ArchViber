...
function brainstormCompactZh(round: number, isConverging: boolean, roundGuidance: string): string {
  ...
  return [
    ...
    '```',
    '',
    '可选字段：`multi: true`（多选/checkbox）、`ordered: true`（带排序 ①②③，隐含 multi）、`min`/`max`（多选上下限）、`allowCustom: true`（追加"其他（自己填）"输入框）、`allowIndifferent: true`（追加"无所谓"选项）。',
    '',
...
function brainstormCompactEn(round: number, isConverging: boolean, roundGuidance: string): string {
  ...
  return [
    ...
    '```',
    '',
    'Optional fields: `multi: true` (checkbox), `ordered: true` (ranked ①②③, implies multi), `min`/`max` (picks bounds), `allowCustom: true` (append a free-text input), `allowIndifferent: true` (append a "doesn\'t matter" option).',
    '',
