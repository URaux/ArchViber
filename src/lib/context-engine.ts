...
function layerOutputFormat(agentType: AgentType, task: TaskType, locale: Locale, sessionPhase?: SessionPhase, brainstormRound?: number, backend?: ContextOptions['backend']): string {
  ...
  if (sessionPhase === 'brainstorm') {
    ...
    const bodyEn = [
      ...
      '```',
      '',
      '**Default: `multi: true, ordered: true`** — most requirement-gathering questions are "rank by importance/relevance and pick several"; the ordering itself is a signal. Use single-select (`multi: false`) only for genuinely mutually exclusive questions (yes/no, choose ONE tech stack, strictly exclusive modes).',
      '',
